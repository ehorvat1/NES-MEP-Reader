...
//#include "html/minuteplot_json.h" 				//EHorvat new 5 Minute plot
...
#endif
extern unsigned long mep_Last_ok_millis;
	...
	if(server.hasArg(F("m")) && server.arg(F("m")) == F("true")) {
		...
		if(server.hasArg(F("me")) && server.arg(F("me")) == F("true")) {
			...
			if(!encryptionKeyHex.isEmpty()) {
				...
				fromHex(meterConfig->encryptionKey, encryptionKeyHex, 21); //EHorvat 21 was 16
			} else {
