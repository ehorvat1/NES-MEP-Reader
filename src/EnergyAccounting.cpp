...
bool EnergyAccounting::update(AmsData* amsData) {
    ...
    if(!init) {
        ...
        if(debugger->isActive(RemoteDebug::DEBUG)) debugger->printf("(EnergyAccounting) Initializing data at %ld\n", (int64_t) now); //EHorvat NES-MEP %lld to %ld
        if(!load()) {
    ...
    if(!initPrice && eapi != NULL && eapi->getValueForHour(0) != ENTSOE_NO_VALUE) {
        if(debugger->isActive(RemoteDebug::DEBUG)) debugger->printf("(EnergyAccounting) Initializing prices at %ld\n", (int64_t) now); //EHorvat NES-MEP %lld to %ld
        calcDayCost();
    }
