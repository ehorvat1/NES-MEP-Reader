...
void SerialEvent2() {
  	while(Serial2.available() && InputBufferLength < MaxMEPReplyLength) {
		...
		if (Debug.isActive(RemoteDebug::VERBOSE)) debugD("++ Received character code: %i",InputBuffer[InputBufferLength]);    //EHorvat new shows received bytes in lines
    	InputBufferLength++;
...
int lastError = 0;
unsigned long lastMEPinfo = 0; //EHorvat 
...
//    			Serial.printf("Transmitting request at index %i (attempt %i)\r\n",MEPQueueSendIndex,MEPQueue[MEPQueueSendIndex].SendAttempts);//EHorvat disabled
				if(Debug.isActive(RemoteDebug::VERBOSE)) debugD("Sending now request at index %i , this is attempt %i  with a length of %i bytes",MEPQueueSendIndex,MEPQueue[MEPQueueSendIndex].SendAttempts,MEPQueue[MEPQueueSendIndex].RequestLength);    //EHorvat new
...
//					if(Debug.isActive(RemoteDebug::VERBOSE)) debugD("This Requests bytes: %i  %X",MEPQueue[MEPQueueSendIndex].Request[i],MEPQueue[MEPQueueSendIndex].Request[i]);    		//EHorvat new
      		...
      		if(millis()-LastSentMillis > 5000) {     //EHorvat Nes-MEP was 2000
				...
				array_60minplot[index_60minplot] = (ConsumptionData.BT28_Fwd_W - ConsumptionData.BT28_Rev_W);
				avg_60minplot = 0;
				for (unsigned long i = 0; i < 4; i++) {
					avg_60minplot = avg_60minplot + array_60minplot[i];
				}
				if (index_60minplot >= 4) {
					ds.update5Minute(avg_60minplot / 4); //EHorvat put data to new 60 Minute Plot array
					index_60minplot = 0;
...
bool readHanPort() {
	...
	if(Debug.isActive(RemoteDebug::INFO)) debugI("*****  bool readHanPort()    was just running now ******"); //  EHorvat NES-MEP added
/* ++++++++++  EHorvat NES-MEP start
