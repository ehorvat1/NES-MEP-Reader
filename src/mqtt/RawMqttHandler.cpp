...
//    if(data->getPackageTimestamp() > 0) {                                                         //EHorvat NES-MEP disabled
//        mqtt->publish(topic + "/meter/dlms/timestamp", String(data->getPackageTimestamp()));      //EHorvat NES-MEP disabled
//    }                                                                                             //EHorvat NES-MEP disabled
/* EHorvat NES-MEP disabled this section --> all data should be sent always....see lines following this disabled section
    switch(data->getListType()) {
        ...
        case 1:
            if(full || meterState->getActiveImportPower() != data->getActiveImportPower()) {
                ...
                //  EHorvat NES-MEP all data should be sent always... dont send "reactive" data...or use it for other stuff
//    mqtt->publish(topic + "/meter/id", data->getMeterId(), true, 0);              //EHorvat NES-MEP disabled
//    mqtt->publish(topic + "/meter/type", data->getMeterModel(), true, 0);           //EHorvat NES-MEP disabled
    mqtt->publish(topic + "/meter/clock", String(data->getMeterTimestamp()));
//    mqtt->publish(topic + "/meter/Frequency", String(data->getFrequency(), 3)); // EHorvat NESMEP new
//    mqtt->publish(topic + "/meter/import/reactive/accumulated_imp_react", String(data->getReactiveImportCounter(), 3), true, 0);    //EHorvat NES-MEP (was /meter/import/reactive/accumulated")
    mqtt->publish(topic + "/meter/MEP_data_ready", String(data->getReactiveImportCounter(), 0), true, 0);    //EHorvat NES-MEP (was /meter/import/reactive/accumulated")
    mqtt->publish(topic + "/meter/import/active/accumulated_imp_active", String(data->getActiveImportCounter(), 3), true, 0);       //EHorvat NES-MEP (was "/meter/import/active/accumulated"
// Send mep_alivecounter instead of ReactiveExportCounter....
//    mqtt->publish(topic + "/meter/export/reactive/accumulated_exp_react", String(data->getReactiveExportCounter(), 3), true, 0);
    mqtt->publish(topic + "/meter/MEP_alive_counter", String(data->getReactiveExportCounter(), 0), true, 0);    //EHorvat NES-MEP (was "/meter/export/reactive/accumulated")
    mqtt->publish(topic + "/meter/export/active/accumulated_exp_active", String(data->getActiveExportCounter(), 3), true, 0);       //EHorvat NES-MEP (was /meter/export/active/accumulated")
    mqtt->publish(topic + "/meter/l1/current_l1", String(data->getL1Current(), 3));                 //EHorvat NES-MEP (was"/meter/l1/current") and dec to 3 (was 2)
    mqtt->publish(topic + "/meter/l1/voltage_l1", String(data->getL1Voltage(), 3));                 //EHorvat NES-MEP (was "/meter/l1/voltage") and dec to 3 (was 2)
    mqtt->publish(topic + "/meter/l2/current_l2", String(data->getL2Current(), 3));                 //EHorvat NES-MEP (was "/meter/l2/current") and dec to 3 (was 2)
    mqtt->publish(topic + "/meter/l2/voltage_l2", String(data->getL2Voltage(), 3));                 //EHorvat NES-MEP (was "/meter/l2/voltage" and dec to 3 (was 2)
    mqtt->publish(topic + "/meter/l3/current_l3", String(data->getL3Current(), 3));                 //EHorvat NES-MEP (was "/meter/l3/current") and decimals to 3 (was 2)
    mqtt->publish(topic + "/meter/l3/voltage_l3", String(data->getL3Voltage(), 3));                 //EHorvat NES-MEP (was "/meter/l3/voltage") and dec to 3 (was 2)
    mqtt->publish(topic + "/meter/FrequencymHz", String(data->getReactiveExportPower()));    //EHorvat NES-MEP (was "/meter/export/reactive")
    mqtt->publish(topic + "/meter/export/active_exp", String(data->getActiveExportPower()));        //EHorvat NES-MEP (was /meter/export/active")
//    mqtt->publish(topic + "/meter/import/reactive_imp", String(data->getReactiveImportPower()));    //EHorvat NES-MEP (was "/meter/import/reactive")  
    mqtt->publish(topic + "/meter/import/active_imp", String(data->getActiveImportPower()));        //EHorvat NES-MEP (was "/meter/import/active")
                //  EHorvat NES-MEP all data should be sent always...end

    mqtt->publish(topic + "/realtime/import/hour_import", String(ea->getUseThisHour(), 3));         //EHorvat NES-MEP (was "/realtime/import/hour")
    mqtt->publish(topic + "/realtime/import/day_import", String(ea->getUseToday(), 3));             //EHorvat NES-MEP (was "/realtime/import/day") and dec to 3 (was 2)
//    for(uint8_t i = 1; i <= ea->getConfig()->hours; i++) {                                                    //EHorvat NES-MEP  disabled sending peaks via MQTT
//        mqtt->publish(topic + "/realtime/import/peak/" + String(i, 10), String(ea->getPeak(i), 10), true, 0); //EHorvat NES-MEP  disabled sending peaks via MQTT
//    }                                                                                                         //EHorvat NES-MEP  disabled sending peaks via MQTT
//   mqtt->publish(topic + "/realtime/import/threshold", String(ea->getCurrentThreshold(), 10), true, 0);       //EHorvat NES-MEP disabled
//   mqtt->publish(topic + "/realtime/import/monthmax", String(ea->getMonthMax(), 3), true, 0);                 //EHorvat NES-MEP disabled
    mqtt->publish(topic + "/realtime/export/hour_export", String(ea->getProducedThisHour(), 3));                //EHorvat NES-MEP was/realtime/export/hour
    mqtt->publish(topic + "/realtime/export/day_export", String(ea->getProducedToday(), 3));                    //EHorvat NES-MEP (was "/realtime/export/day") and dec to 3 (was 2)
    return true;
}
