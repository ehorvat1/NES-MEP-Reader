...
var sa;
var so = {
    title: 'last 5 minutes',  // ****** EHorvat was Energy use last 5 Minutes
    titleTextStyle: {
        fontSize: 14
    },
...
var ea;
var eo = {
    title: 'last 24 hours',  //EHorvat was Energy use last 24 hours
    titleTextStyle: {
        fontSize: 14
    },
    //   EHorvat .... next 2 Lines....option to exchange "Export" with "Import" (to show Import as "negativ" values on plot)
...
var ma;
var mo = {
    title: 'last month', //EHorvat was  Energy use last month
    titleTextStyle: {
        fontSize: 14
    },
    //   EHorvat .... next 2 Lines....option to exchange "Export" with "Import" (to show Import as "negativ" values on plot)
...
var va;
var vo = {
    title: 'Voltage',   //EHorvat was: Phase voltage
    titleTextStyle: {
        fontSize: 14
    },
    backgroundColor: { fill:'transparent' },
...
var aa;
var ao = {
    title: 'Current',   //EHorvat was: Phase current
    titleTextStyle: {
        fontSize: 14
    },
    backgroundColor: { fill:'transparent' },
...
// Import plot   '#6f42c1' , Export = green '#28a745'
var ip;
var ia;
var io = {
    legend: 'none',
...
var drawMinute = function() {
    $.ajax({
        ...
        for(var i = 59; i > -1; i--) {   // Show current data on right side
            ...
            data[r++] = [((i+1)*5) + " Sec. ago", val, "opacity: 0.9;"]; //  EHorvat ... just one line
...
var drawDay = function() {
    $.ajax({
        ...
        //   EHorvat .... next 2 Lines....option to exchange "Export" with "Import" (to show Import as "negativ" values on plot)
        data = [['Hour', 'Import', { role: 'style' }, 'Export', { role: 'style' }]]; //  EHorvat ... original: Import is positive
        //data = [['Hour', 'Export', { role: 'style' }, { role: 'annotation' }, 'Import', { role: 'style' }, { role: 'annotation' }]];  //EHorvat exchanged Import / Export
        var r = 1;
        var hour = moment.utc().hours();
        var offset = moment().utcOffset()/60;
        ...
        for(var i = hour; i<24; i++) {
            ...
            //   EHorvat .... next 4 Lines....option to exchange "Export" with "Import" (to show Import as "negativ" values on plot)            
            data[r++] = [zeropad((i+offset)%24), imp, "opacity: 0.9;", exp == 0 ? 0 : -exp, "opacity: 0.9;"];//EHorvat ... original: Import is positive
            //data[r++] = [zeropad((i+offset)%24), exp, "opacity: 0.9;", exp == 0 ? "" : exp.toFixed(2), imp == 0 ? 0 : -imp, "opacity: 0.9;", imp == 0 ? "" : -imp.toFixed(2)];//EHorvat ... modified: Import is negative
...
var drawMonth = function() {
    $.ajax({
        ...
        //   EHorvat .... next 2 Lines....option to exchange "Export" with "Import" (to show Import as "negativ" values on plot)
        data = [['Hour', 'Import', { role: 'style' }, 'Export', { role: 'style' }]]; //  EHorvat ... original: Import is positive
        //data = [['Hour', 'Export', { role: 'style' }, { role: 'annotation' }, 'Import', { role: 'style' }, { role: 'annotation' }]]; //  EHorvat ... modified: Import is negative
        var r = 1;
        var day = moment().date();
        ...
        for(var i = day; i<=eom; i++) {
            ...
            var exp = json["e"+zeropad(i)];
            //  EHorvat .... next 4 Lines....option to exchange "Export" with "Import" (to show Import as "negativ" values on plot)
            data[r++] = [zeropad(i), imp, "opacity: 0.9;", exp == 0 ? 0 : -exp, "opacity: 0.9;"]; //  EHorvat ... original: Import is positive
            //data[r++] = [zeropad(i), exp, "opacity: 0.9;", exp == 0 ? "" : exp.toFixed(1), imp == 0 ? 0 : -imp, "opacity: 0.9;", imp == 0 ? "" : -imp.toFixed(1)]; //  EHorvat ... modified: Import is negative
            min = Math.min(0, -exp);   //EHorvat ... original: Import is positive
            //min = Math.min(0, -imp); //EHorvat ... modified: Import is negative
        }
        for(var i = 1; i < day; i++) {
        ...
        if(vp) {
            switch(ds) {
                case 1:
                    ...
                    break;
                case 2:
                    vo.title = 'Voltage'; //EHorvat was: Phase voltage
                    break;
            }
            var c = 0;
            var t = 0;
        ...
        if(ap && json.mf) {
            switch(ds) {
                case 1:
                    ...
                    break;
                case 2:
                    ao.title = 'Current';   //EHorvat was:  Phase current
                    break;
            }
            var dA = false;
            var r = 1;
