...
bool AmsConfiguration::getSystemConfig(SystemConfig& config) {
	...
	if(configVersion == EEPROM_CHECK_SUM || configVersion == EEPROM_CLEARED_INDICATOR) {
		...
		config.boardType = 0xC8;           //EHorvat 0xC8 = 200 dez = Generic ESP32(was 0xFF)
		config.vendorConfigured = false;
		config.userConfigured = false;
		config.dataCollectionConsent = 2;  //EHorvat (was 0)
...
bool AmsConfiguration::setMeterConfig(MeterConfig& config) {
	...
	bool ret = EEPROM.commit();
	EEPROM.end();
	config.baud = 9600; //EHorvat new ... set baud rate to 9600 even if not so in EEPROM
	config.parity = 3; //EHorvat new ... set parity to 8N1 (11 would be 8E1)
	return ret;
}

void AmsConfiguration::clearMeter(MeterConfig& config) {
	config.baud = 9600; //EHorvat (was 0)
	config.parity = 3; //EHorvat (was 0) ... set parity to 8N1 (11 would be 8E1)
	config.invert = false;
	config.distributionSystem = 2;
	config.mainFuse = 40;
	config.productionCapacity = 0;
	memset(config.encryptionKey, 1, 21);   // EHorvat was "0, 16" preset to "1" fills in an ecryption default key and meter encryption gets checked.
...
bool AmsConfiguration::getGpioConfig(GpioConfig& config) {
	...
	if(configVersion == EEPROM_CHECK_SUM || configVersion == EEPROM_CLEARED_INDICATOR) {
		...
		EEPROM.end();
		config.apPin = 26;   //EHorvat new set AP GPIO to 26 ... even EEPROM has different value
	    config.hanPin = 16;  //EHorvat new set 16 for UART2 on ESP32... even EEPROM has different value
		return true;
	} else {
		clearGpio(config);
		return false;
...
void AmsConfiguration::clearGpio(GpioConfig& config) {
	config.hanPin = 16;  //EHorvat set 16 for UART2 on ESP32 (was 3)
	config.hanPinPullup = true;
	config.apPin = 26; //EHorvat set AP GPIO to 26 (was 0xFF)
	config.ledPin = 0xFF;
...
void AmsConfiguration::clearUiConfig(UiConfig& config) {
	...
	config.showImport = 1;
	config.showExport = 2;
	config.showVoltage = 1;
	config.showAmperage = 1;
	config.showReactive = 0;
	config.showLast_Counter = 1;  //EHorvat new last_counte_value
...
void AmsConfiguration::print(Print* debugger)
{
	...
	uint16_t chipId;  //EHorvat new get ChipID also here
	...
	debugger->println(F("--AP Wifi configuration--"));//EHorvat new
	debugger->printf_P(PSTR("Wifi AP :             '%s'\r\n"), (String("Stromzaehler-") + String(chipId, HEX)).c_str());  //EHorvat new
	debugger->printf_P(PSTR("AP Passw:             '%s'\r\n"), WIFI_AP_PASSWORD);     //EHorvat new
	debugger->println(F(""));//EHorvat new
	delay(10);//EHorvat new
	debugger->flush();//EHorvat new
