...
#endif
extern unsigned long mep_Last_ok_millis;    //EHorvat new variable
...
//	bool access = WiFi.getMode() == WIFI_AP || webConfig.security < level;
	bool access = webConfig.security < level;  //EHorvat ... enforce security also when in AP mode
...
void AmsWebServer::sysinfoJson() {
	...
	int size = snprintf_P(buf, BufferSize, SYSINFO_JSON,
		...
		ui.showAmperage,
		ui.showReactive,
		ui.showLast_Counter,// EHorvat new last_counter_value ... also change sysinfo.json
		ui.showRealtime,
		ui.showPeaks,
		ui.showPricePlot,
...
//		ui.showTemperaturePlot,
		ui.showFrequency,  // EHorvat new frequency indicator
		webConfig.security,
		#if defined(ESP32)
...
//	} else if(millis - meterState->getLastUpdateMillis() < 15000) {
	} else if(millis - mep_Last_ok_millis < 15000) {    			// EHorvat new Timestamp to be used for HAN/MEP port timeout error indication
		hanStatus = 1;  //EHorvat this is to show lable in orange
//	} else if(millis - meterState->getLastUpdateMillis() < 30000) {
	} else if(millis - mep_Last_ok_millis < 30000) {    			// EHorvat new Timestamp to be used for HAN/MEP port timeout error indication
		hanStatus = 2; //EHorvat this is to show lable in red
	} else {
		hanStatus = 3;//EHorvat this is to show lable in green
	}
...
//		hw->getTemperature(),
		...
		refund == ENTSOE_NO_VALUE ? "null" : String(refund, 2).c_str(),		//EHorvat new fixed income price / refund Link in data.json:"pi" : %s,
...
void AmsWebServer::dayplotJson() {
	...
	if(!checkSecurity(2))
		return;
	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("Buffer length actual of day.json...is: %d, \n"), strlen(buf));//EHorvat
	if(ds == NULL) {
		notFound();
	} else {
...
void AmsWebServer::monthplotJson() {
	...
	if(!checkSecurity(2))
		return;
	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("Buffer length actual of month.json...is: %d, \n"), strlen(buf));//EHorvat
	if(ds == NULL) {
		notFound();
	} else {
...
//	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("++++++++getHour-0 : %i\n"), ds->getHourWatt(0));  //EHorvat changed to Info level
//	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("++++++++getHour-179 : %i\n"), ds->getHourWatt(179));  //EHorvat changed to Info level
...
void AmsWebServer::configurationJson() {
	...
	bool encen = false;
	for(uint8_t i = 0; i < 21; i++) {				//EHorvat 21 was 16
	...
	snprintf_P(buf, BufferSize, CONF_METER_JSON,
		...
		meterConfig->productionCapacity,
		encen ? "true" : "false",
		toHex(meterConfig->encryptionKey, 21).c_str(),			//EHorvat 21 was 16
	...
	snprintf_P(buf, BufferSize, CONF_UI_JSON,
		...
		ui.showAmperage,
		ui.showReactive,
		ui.showLast_Counter,   //EHorvat new last_counte_value 
		ui.showRealtime,
		ui.showPeaks,
		ui.showPricePlot,
		ui.showMinutePlot,		
...
void AmsWebServer::handleSave() {
	...
	if(server.hasArg(F("s")) && server.arg(F("s")) == F("true") && server.hasArg(F("ss")) && !server.arg(F("ss")).isEmpty()) {
		...
		sys.dataCollectionConsent = 2;  //EHorvat (was 0)
		config->setSystemConfig(sys);
	...
	if(server.hasArg(F("m")) && server.arg(F("m")) == F("true")) {
		...
		if(server.hasArg(F("me")) && server.arg(F("me")) == F("true")) {
			...
			if(!encryptionKeyHex.isEmpty()) {
				...
				fromHex(meterConfig->encryptionKey, encryptionKeyHex, 21); //EHorvat 21 was 16; convert encryption key from web page to meterConfig
			} else {
				memset(meterConfig->encryptionKey, 0, 21); //EHorvat 21 was 16.....empty encryprion key, all zero
			}
			...
			memset(meterConfig->encryptionKey, 0, 21); //EHorvat 21 was 16
			memset(meterConfig->authenticationKey, 0, 16);
		}
	...
	if(server.hasArg(F("u")) && server.arg(F("u")) == F("true")) {
		...
		ui.showLast_Counter = server.arg(F("ul")).toInt(); //EHorvat new last_counte_value 
...
//		ui.showTemperaturePlot = server.arg(F("us")).toInt();
		ui.showFrequency = server.arg(F("uf")).toInt(); //EHorvat new frequency indicator
