        ...
        fetchWithTimeout("/data.json")
            ...
            .then((data) => {
                ...
                if(sysinfo.upgrading) {
                    ...
                    minutePlotTimeout = setTimeout(getMinutePlot, 2000);    //EHorvat new hour plot
                ...
                let to = 2000; //EHorvat changed to 2000 (was 5000)  faster web data update
                if(isBusPowered(sysinfo.board) && data.v > 2.5) {
                    let diff = (3.3 - Math.min(3.3, data.v));
                    if(diff > 0) {
                        to = Math.max(diff, 0.1) * 10 * 2000;//EHorvat changed to 2000 (was 5000)  faster web data update
                    }
                }
                if(to > 2000) console.log("Scheduling next data fetch in " + to + "ms");//EHorvat changed to 2000 (was 5000)  faster web data update
            ...
            .catch((err) => {
                ...
                if(tries > 3) {
                    ...
                    timeout = setTimeout(getData, isBusPowered(sysinfo.board) ? 10000 : 2000);//EHorvat changed to 2000 (was 5000)  faster web data update
                }
            });
    }
    getData();
...
minutePlotTimeout = setTimeout(getMinutePlot, 2000)  //Update every 2 sec
}
...
export async function getHourPlot() {
    ...
    let date = new Date();
//    hourPlotTimeout = setTimeout(getHourPlot, ((60-date.getSeconds())*1000)+30)
